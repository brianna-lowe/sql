...
SELECT
	...
	ROW_NUMBER()OVER(PARTITION BY customer_id ORDER BY market_date) visit_number
...
hypothetical_sales AS (
  SELECT
    x.vendor_id,
    x.product_id,
    5 * x.original_price 	AS revenue_per_customer
  FROM vendor_products x
  CROSS JOIN all_customers y
),
...
-- CREATE SCHEMA IF NOT EXISTS temp; (not necessary)
...
WHERE product_name IN ('Sour Bri Saurkraut', 'Sour Bri Sauerkraut')
ORDER BY snapshot_timestamp;

--deleted mispelled product names

DELETE FROM temp.product_units
WHERE product_id = 24
  AND product_name = 'Sour Bri Saurkraut'
...
SET current_quantity = COALESCE(
  (
    ...
    WHERE lp.product_id = pu.product_id
      AND lp.rn = 1
  ),
  0
);
...
--- I have no idea if this is working, I am so tired. Sorry for taking so long with this assignemnt, it is pretty tough.